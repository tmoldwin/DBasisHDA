...
If you want to use SHD as a subroutine, there is a way. First, you
have to define _FILE2_LOAD_FROM_MEMORY_, and include SHD by
#include"shd.c". Then, SHD load the data from memory, pointed by a
(void *) pointer __load_from_memory_org__ (this is automatically defined 
in shd.c). The numbers to give to SHD is directly and sequentially has
to be stored in the memory. For example, if the input file is 

12 24 0.333
0 200 0.444
[EOF]

Then, we have to store these numbers, separator, and endmarks, one by
one; (int)12 (int)24 (double)0.333 (int)XXXX (int)0 (int)200
(double)0.444 (int)(XXXX-1)
where XXXX is an endmark representing the end of line, and XXXX-1
represents the end of the file. XXXX is defined by
FILE2_LOAD_FROM_MEMORY_END, and int the default, it is set to INTHUGE,
that is an almost highest number in the number type (for interger, 
2,000,000,000). Before including "shd.c", you can define 
FILE2_LOAD_FROM_MEMORY_END as you like, then default number will not
be used.

To call shd, first, set pointer __load_from_memory_org__ to the top
of the data array, then use EXECSUB macro, for example, 

  EXECSUB (SHD_main, flag, exit, "shd 0 -l %d dummy dummy", lower_bound);

This is an emulator of command line.
"SHD_main" is the main routine of SHD, so it has to be this. "flag" is
whether you want to show a message to standard output that the program
calls SHD_main. "exit" is the command to execute when some errors
occur (you can also give like "printf ("error!\n"); exit (0)", because
it is a macro). The forth parameter, string, is the sentence to be
typed to execute SHD as usual. Actually, sprintf is executed for the
string and the following parameters, so that you can give numbers in
the variables to SHD. For example, "SHD 9 -u %d -l %d dummy dummy", 5, 2"
will be "SHD 9 -u 5 -l 2 dummy dummy", and it is given to SHD.
You have to give a dummy filename as input file name to SHD, even
though it is not used by SHD in this case, otherwise an error occur, at 
the begining of SHD_main.
You have to give at least one object as the fifth parameter, even though
you need no parameter for the sprintf, otherwise compile error occurs.

This is the general case, for all Uno's implementations. In particular,
in SHD, if file is edge type, then

12 24
11 45
...
will be 
12 24 INTHUGE 11 45 INTHUGE ... INTHUGE-1

The output will be written to __write_to_memory_org__, and it is also
automatically defined in shd.c. The output is written to the array
pointed by __write_to_memory_org__, and it is automatically allocated.
__write_to_memory_org__ is a pointer of "char *"
The format of the array is the same as input file, so one line in 
the output file (representing one hitting set) is stored as a
consecutive integers in the memory, and at the end of a line, we 
have FILE2_LOAD_FROM_MEMORY_END. At the last of the array, that 
corresponds to the end of the file, we have FILE2_LOAD_FROM_MEMORY_END-1.
So, we have consecutive, FILE2_LOAD_FROM_MEMORY_END and
FILE2_LOAD_FROM_MEMORY_END-1. An example is in sample.c.
